...
``jobpipe/tailor/templates/VOICE_PROFILE.md``. After the move to
...
def _tailor_templates_dir() -> Path:
    ...
    file moved from jobpipe/tailor/applicant/ (where ../templates worked)
    to jobpipe/submit/adapters/. Walks up to find pyproject.toml so the
    lookup is independent of where this module physically lives.
    """
    here = Path(__file__).resolve()
    for candidate in (here, *here.parents):
        if (candidate / "pyproject.toml").is_file():
            return candidate / "jobpipe" / "tailor" / "templates"
    # Fallback: assume the canonical layout if pyproject.toml isn't found.
    return here.parents[3] / "jobpipe" / "tailor" / "templates"
